...
# ---------------------------------------------------------------- portfolio
def compute_portfolio(positions, cash, goal, external_total=0):
    """positions: list of dicts with entry, price, shares (+ optional sector).
    external_total: combined value of your OTHER accounts (India book, MFs,
    401k, HSA, NPS...) -- everything OUTSIDE this US trading sheet. It is added
    to the headline so Account Value reflects your real combined total without
    double-counting the US sleeve. Defaults to 0 (trading account only).
    ...
    trading_value = invested + (cash or 0)
    external_total = external_total or 0
    total_value = trading_value + external_total
    gain = invested - cost
    ...
    return {
        "goal": goal,
        "total_value": round(total_value, 2),       # combined: this account + everything else
        "trading_value": round(trading_value, 2),   # this US trading account only (live)
        "external_total": round(external_total, 2),  # all other accounts (entered/loaded)
        "cash": round(cash or 0, 2),
        "invested": round(invested, 2),
        "cost_basis": round(cost, 2),
        "total_gain": round(gain, 2),                # gain figures are trading-account scoped
...
def sell_signals(positions, targets=None, run_up=25, stop=-15):
    ...
    NOTE: build_dashboard filters this list against the rules engine, so a
    signal here only surfaces if the engine agrees. The engine (your Rules tab)
    is the single source of truth for cut/hold."""
    targets = targets or {}
    out = []
    for p in positions:
        tk = p["ticker"]
